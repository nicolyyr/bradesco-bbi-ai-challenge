...
└── llm/
    ...
    ├── providers.py      # LLMProvider ABC, OpenAIProvider, MockProvider
...
| `providers.py` | talk to OpenAI / emulate offline | validate business schema |
...
sequenceDiagram
    ...
    participant P as Provider (OpenAI|Mock)
...
- **Transport:** OpenAI `chat.completions` with `response_format={"type":
  "json_object"}` to bias toward valid JSON. The SDK is imported lazily so mock
  runs and tests never require it.
...
- **Testability:** `OpenAIProvider` accepts an injected client, so the real code
  path is unit-tested with a fake (no network, no key).
